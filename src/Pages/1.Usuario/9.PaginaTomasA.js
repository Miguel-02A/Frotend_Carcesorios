...
const TomasDeAire = () => {
    ...
    useEffect(() => {
        const getTitulo = () => {
            fetch('http://localhost:4000/carcesorios/PaginaTomasAire/Titulo')
                .then(res => res.json())
    ...
    useEffect(() => {
        const Productos = async () => {
            try {
                const response = await axios.get('http://localhost:4000/carcesorios/PaginaTomasAire');
            ...
            <section id="ListaProductos">
                {productos.map((producto, index) => (
                    <div className="caja" key={index}>
                        <a href='/DescripcionProducto' onClick={() => idDelProducto(producto.id_producto)}><img src={producto.url_imagen_producto} alt={producto.id_producto} /><h2>{producto.nombre_producto}</h2><h4>{producto.precio}$</h4></a>
                    </div>
                ))}
            </section>
