...
const Ofertas = () => {
        ...
        useEffect(() => {
                const getTitulo = () => {
                        fetch('http://localhost:4000/carcesorios/Ofertas/Titulos')
                                .then(res => res.json())
        ...
        useEffect(() => {
                const Productos = async () => {
                        try {
                                const response = await axios.get('http://localhost:4000/carcesorios/Ofertas');
                        ...
                        <section id="ListaProductos">
                                {productos.map((producto, index) => (
                                        <div className="caja" key={index}>
                                                <a href='/DescripcionProductoCliente' onClick={() => idDelProducto(producto.id_producto)}><img src={producto.url_imagen_producto} alt={producto.id_producto} /><h2>{producto.nombre_producto}</h2><h4>{producto.precio} - {producto.descuento}%desc</h4></a>
                                        </div>
                                ))}
                        </section>
